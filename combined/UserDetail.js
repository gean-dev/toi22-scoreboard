...
var UserDetail = new function () {
    ...
    self.do_show = function () {
        if (self.data_fetched >= 2) {
            ...
            var name_parts = self.user["l_name"].trim().split(/\s+/);
            if (name_parts.length >= 3) {
                // Assuming format: Title First Last
                var firstName = name_parts[1];
                var lastName = name_parts.slice(2).join(" ");
                self.l_name_label.html(escapeHTML(firstName) + "<br/>" + escapeHTML(lastName));
            } else if (name_parts.length === 2) {
                // Assuming format: Title First (or First Last?)
                // User said remove title, so we take the second part.
                self.l_name_label.text(name_parts[1]);
            } else {
            ...
            if (self.user["team"]) {
                if (true || DataStore.asset_config && DataStore.asset_config["noflags"])
