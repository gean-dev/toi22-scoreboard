...
<tr> \
    ...
    <th colspan=\"5\" class=\"f_name\">CID</th> \
    <th colspan=\"10\" class=\"l_name\">Name</th> \
    <th class=\"user_id\">ID</th> \
    <th colspan=\"2\" class=\"team\">Team</th>";
...
<tr class=\"user" + (user["selected"] > 0 ? " selected color" + user["selected"] : "") + "\" data-user=\"" + user["key"] + "\"> \
    ...
    <td colspan=\"5\" class=\"f_name\">" + escapeHTML(user["f_name"]) + "</td> \
    <td colspan=\"10\" class=\"l_name\">" + escapeHTML(user["l_name"].split(" ").slice(1).join(" ")) + "</td> \
    <td class=\"user_id\">" + user["key"] + "</td>";

        if (user['team']) {
            if (DataStore.asset_config && DataStore.asset_config["noflags"])
                result += " \
        <td colspan=\"2\" class=\"team\">" + user["team"] + "</td>";
            else
                result += " \
...
<td class=\"team\"></td>";
    ...
    self.update_user = function (u_id, old_user, user) {
        ...
        $row.children("td.l_name").text(user["l_name"].split(" ").slice(1).join(" "));
